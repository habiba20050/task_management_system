...
class _EvaluationsPageState extends State<EvaluationsPage>
  ...
  Widget build(BuildContext context) {
    ...
    final teamName =
        team.name.isEmpty ? (team.department.isEmpty ? 'My Team' : team.department) : team.name;
    ...
    final sortedMembers = List<MockUser>.from(teamMembers)
    ...
    final memberIds = team.memberIds.toSet();
    final teamCompleted = db.tasks
        .where((t) =>
            memberIds.contains(t.currentOwnerId) &&
            (t.status == 'Completed' || t.status == 'Approved'))
        .length;
    final teamOverdue =
        db.tasks.where((t) => memberIds.contains(t.currentOwnerId) && t.status == 'Overdue').length;

    final leaderMock = db.users.cast<MockUser?>().firstWhere(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          'Team Performance & Evaluations'.tr(context),
                          style: TextStyle(
                            fontSize: 22.sp,
                            fontWeight: FontWeight.bold,
                            color: AppColors.textPrimary,
                        ...
                        SizedBox(height: 3.h),
                        Text(
                          '$teamName · ${'Member Rankings & Scores'.tr(context)}',
                          style: TextStyle(
                            fontSize: 13.sp,
                        ...
                        child: ResponsiveLayout.isMobile(context)
                            ...
                            : Row(
                                ...
                                children: [
                                  ...
                                  SizedBox(width: 8),
                                  Flexible(
                                    child: Text('Team Overview'.tr(context),
                ...
                child: TabBarView(
                  ...
                  children: [
                    ...
                    _buildTeamTab(
                      ...
                      teamOverdue: teamOverdue,
                    ),

                    // Tab 2: Leader's own score
                    _buildMyScoreTab(context, leaderMock),
                  ],
                ),
              ),
            ],
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          LayoutBuilder(builder: (context, constraints) {
            ...
            return GridView.count(
              ...
              children: [
                ...
                _kpiCard(context, 'Top Score'.tr(context),
                    bestMember == null
                        ? 'N/A'
                        : '${bestMember.finalScore.toStringAsFixed(1)}%',
                    Icons.emoji_events_outlined, Colors.amber),
                _kpiCard(context, 'Completed Tasks'.tr(context), teamCompleted.toString(),
                    Icons.task_alt_outlined, AppColors.success),
                _kpiCard(context, 'Overdue Tasks'.tr(context), teamOverdue.toString(),
                    Icons.timer_off_outlined, AppColors.danger),
              ],
            );
          }),
          ...
          // ── Best / Worst Rankings ──
          _sectionTitle('Team Rankings'.tr(context)),
          ...
          LayoutBuilder(builder: (context, constraints) {
            ...
            return GridView.builder(
              ...
              itemBuilder: (context, i) {
                return _rankingCard(
                  context,
                  i == 0 ? 'Best Team Member'.tr(context) : 'Needs Improvement (Member)'.tr(context),
                  i == 0 ? bestMember : worstMember,
                  i == 0 ? Icons.emoji_events_outlined : Icons.trending_down,
                  i == 0 ? AppColors.success : AppColors.danger,
                );
              },
            );
          }),
          ...
          sortedMembers.isEmpty
              ? _emptyState(context, 'No team members in your team'.tr(context))
  ...
  // ─── TAB 2: Leader's own score ───────────────────────────────────────────
  Widget _buildMyScoreTab(BuildContext context, MockUser? leaderMock) {
    if (leaderMock == null) {
      return Center(child: Text('Leader data not found.'.tr(context)));
    }

    final scoreColor = _getScoreColor(leaderMock.finalScore);
    final db = MockDatabase.instance;
    final completedTasks = db.tasks
        .where((t) => t.currentOwnerId == leaderMock.id && (t.status == 'Completed' || t.status == 'Approved'))
        .length;
    final activeTasks = db.tasks
        .where((t) => t.currentOwnerId == leaderMock.id && t.status != 'Completed' && t.status != 'Approved')
        .length;
    final evalHistory =
        db.evaluations.where((e) => e.employeeId == leaderMock.id).toList();
      ...
      child: Column(
        ...
        children: [
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: CircularProgressIndicator(
                        value: leaderMock.finalScore / 100.0,
                        strokeWidth: 7.r,
                        backgroundColor: const Color(0xFFF1F5F9),
                        color: scoreColor,
                      ),
                    ),
                    _avatar(leaderMock.fullName, 78, scoreColor),
                  ],
                );
                final info = Column(
                  crossAxisAlignment:
                  ...
                  children: [
                    ...
                    SizedBox(height: 3.h),
                    Text(
                      '${'Team Leader'.tr(context)} · ${leaderMock.department}',
                    ...
                    Container(
                      ...
                      child: Text(
                        _getScoreLabel(leaderMock.finalScore).tr(context),
                ...
                final scoreBox = Column(
                  ...
                  children: [
                    Text(
                      '${leaderMock.finalScore.toStringAsFixed(1)}%',
          ...
          LayoutBuilder(builder: (context, constraints) {
            ...
            final metrics = [
              _MetricData('Productivity'.tr(context), leaderMock.productivityScore, Icons.speed_outlined, AppColors.primary),
              _MetricData('Deadline Commit.'.tr(context), leaderMock.deadlineCommitment, Icons.timer_outlined, Colors.indigo),
              _MetricData('Approval Rate'.tr(context), leaderMock.approvalRate, Icons.check_circle_outline, AppColors.success),
              _MetricData('Leader Eval.'.tr(context), leaderMock.leaderEvaluation, Icons.star_border_outlined, Colors.amber),
              _MetricData('Rejection Rate'.tr(context), leaderMock.rejectionRate, Icons.cancel_outlined, AppColors.danger),
              ...
              _MetricData('Points'.tr(context), leaderMock.points.toDouble(), Icons.emoji_events_outlined, Colors.amber),
            ];
            return GridView.builder(
              shrinkWrap: true,
              physics: const NeverScrollableScrollPhysics(),
