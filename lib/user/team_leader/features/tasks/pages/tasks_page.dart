...
class _TasksPageState extends State<TasksPage> {
  String _selectedSection = 'My Tasks'; // 'My Tasks' | 'Team Tasks'
  ...
  // --- Fetch Tasks matching filters + section scope ---
  List<UnifiedItem> _getFilteredAndSortedTasks(String currentUserId, String currentUserRole) {
    ...
    for (final task in db.tasks) {
      ...
      // Section-based filtering (My Tasks vs Team Tasks)
      if (currentUserRole == 'Team Leader') {
        if (_selectedSection == 'My Tasks' && task.currentOwnerId != currentUserId) {
          continue;
        }
        if (_selectedSection == 'Team Tasks' && task.currentOwnerId == currentUserId) {
          continue;
        }
      }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 4.h),
                        Text(
                          _selectedSection == 'My Tasks'
                              ? 'Your personal tasks and assignments'.tr(context)
                              : 'Manage and delegate team academic tasks'.tr(context),
                          style: TextStyle(fontSize: 12.sp, color: AppColors.textSecondary),
                        ),
                      ],
                    ),
                  ),
                  if (_selectedSection == 'Team Tasks' && (role == 'Admin' || role == 'Manager' || role == 'Team Leader'))
                    Padding(
              ...
              // Section Toggle: My Tasks | Team Tasks
              if (role == 'Team Leader')
                Padding(
                  ...
                  child: ToggleButtons(
                    ...
                    constraints: BoxConstraints(minWidth: 120.w, minHeight: 32.h),
                    isSelected: [_selectedSection == 'My Tasks', _selectedSection == 'Team Tasks'],
                    onPressed: (index) {
                      setState(() {
                        _selectedSection = index == 0 ? 'My Tasks' : 'Team Tasks';
                      });
                    },
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          SizedBox(width: 4.w),
                          Text('My Tasks'.tr(context), style: TextStyle(fontSize: 11.sp, fontWeight: FontWeight.bold)),
                        ],
                      ),
                      Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          ...
                          SizedBox(width: 4.w),
                          Text('Team Tasks'.tr(context), style: TextStyle(fontSize: 11.sp, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ],
                  ),
  ...
  Widget _buildFilterBar(BuildContext context) {
    ...
    final teamMemberIds = myTeam != null ? [currentUserId, ...myTeam.memberIds] : <String>[currentUserId];
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              if (_selectedSection == 'Team Tasks')
                _buildFilterField(
                  label: 'Member',
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: [
                ...
                SizedBox(width: 6.w),
                Text(value, style: TextStyle(fontSize: 11.sp, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                SizedBox(width: 4.w),
      ...
      child: ClipRRect(
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            final colW = <String, double>{
              'title': isWide ? 180.w : (isMedium ? 140.w : 100.w),
              'priority': 70.w,
              'status': 90.w,
              'startDate': isWide ? 100.w : 0,
              'dueDate': 100.w,
              'remaining': isWide ? 90.w : 0,
              'progress': isWide ? 80.w : (isMedium ? 60.w : 0),
              'createdDate': isWide ? 100.w : 0,
              'actions': 120.w,
            };
            ...
            return SingleChildScrollView(
              ...
              child: SingleChildScrollView(
                ...
                child: DataTable(
                  ...
                  columnSpacing: 8,
                  columns: [
                    _buildSortableColumn('Task Title', 'Task Title', width: colW['title']!),
                    _buildSortableColumn('Priority', 'Priority', width: colW['priority']!),
                    _buildSortableColumn('Status', 'Status', width: colW['status']!),
                    if (isWide || isMedium)
                      _buildSortableColumn('Start Date', 'Start Date', width: colW['startDate']!),
                    _buildSortableColumn('Due Date', 'Due Date', width: colW['dueDate']!),
                    if (isWide)
                      _buildSortableColumn('Remaining Time', 'Remaining Time', width: colW['remaining']!),
                    if (isWide || isMedium)
                      _buildSortableColumn('Progress', 'Progress', width: colW['progress']!),
                    if (isWide)
                      _buildSortableColumn('Created Date', 'Created Date', width: colW['createdDate']!),
                    DataColumn(label: SizedBox(width: colW['actions'], child: Text('Actions'.tr(context), style: const TextStyle(fontWeight: FontWeight.bold)))),
                  ],
                  rows: items.map((item) {
                    final task = item.originalObject as MockTask;
                    ...
                    return DataRow(cells: [
                      DataCell(SizedBox(width: colW['title'], child: Text(task.title, style: const TextStyle(fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis))),
                      DataCell(SizedBox(
                        width: colW['priority'],
                        child: Container(
                      ...
                      DataCell(SizedBox(width: colW['status'], child: Text(task.status.tr(context), overflow: TextOverflow.ellipsis))),
                      if (isWide || isMedium)
                        DataCell(SizedBox(width: colW['startDate'], child: Text(task.startDate, overflow: TextOverflow.ellipsis))),
                      DataCell(SizedBox(width: colW['dueDate'], child: Text(task.deadline, overflow: TextOverflow.ellipsis))),
                      if (isWide)
                        DataCell(SizedBox(width: colW['remaining'], child: Text(remaining, style: TextStyle(color: remaining == 'Overdue'.tr(context) ? AppColors.danger : AppColors.textPrimary)))),
                      if (isWide || isMedium)
                        DataCell(SizedBox(width: colW['progress'], child: LinearProgressIndicator(value: progress, color: progress == 1.0 ? AppColors.success : AppColors.primary))),
                      if (isWide)
                        DataCell(SizedBox(width: colW['createdDate'], child: Text(_getCreatedDate(task), overflow: TextOverflow.ellipsis))),
                      DataCell(SizedBox(
                        width: colW['actions'],
                        child: SingleChildScrollView(
                          scrollDirection: Axis.horizontal,
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              _smallIconButton(Icons.visibility, Colors.blue, () => _showQuickViewModal(context, task)),
                              _smallIconButton(Icons.edit, Colors.orange, () => _showEditTaskDialog(context, task, currentUserId)),
                              _smallIconButton(Icons.delete, AppColors.danger, () => _confirmDeleteTask(context, task.id)),
                              if (_selectedSection == 'Team Tasks' && (task.status == 'Submitted' || task.status == 'Under Review') && task.currentOwnerId != currentUserId)
                                _smallIconButton(Icons.rate_review, Colors.teal, () => _showTeamTaskReviewDialog(context, task), tooltip: 'Evaluate'.tr(context))
                              else if (task.status == 'Submitted' || task.status == 'Completed' || task.status == 'Approved')
                                _smallIconButton(Icons.assignment_turned_in, Colors.green, () => _showViewSubmissionDialog(context, task), tooltip: 'View Submission'.tr(context))
                              else
                                _smallIconButton(Icons.send, Colors.purple, () => _showSubmitTaskDialog(context, task), tooltip: 'Submit Task'.tr(context)),
                            ],
                          ),
                        ),
                      )),
  ...
  DataColumn _buildSortableColumn(String label, String field, {double? width}) {
    ...
    return DataColumn(
      label: InkWell(
        ...
        child: SizedBox(
          ...
          child: Row(
            children: [
              Flexible(child: Text(label.tr(context), style: const TextStyle(fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis)),
              if (isSelected) ...[
                SizedBox(width: 4.w),
  ...
  Widget _smallIconButton(IconData icon, Color color, VoidCallback onPressed, {String? tooltip}) {
    return IconButton(
        ...
        actions: [
          ...
          if (_selectedSection == 'Team Tasks' && (task.status == 'Submitted' || task.status == 'Under Review') && task.currentOwnerId != currentUserId)
            ElevatedButton(
              ...
              child: Text('Evaluate'.tr(context)),
            )
          else if (task.status == 'Submitted' || task.status == 'Completed' || task.status == 'Approved')
            ElevatedButton(
              onPressed: () {
  ...
  void _showAddTaskDialog(BuildContext context, String currentUserId) {
    ...
    final teamMembers = leaderTeam != null
        ? db.users.where((u) => leaderTeam.memberIds.contains(u.id)).toList()
        : <MockUser>[];
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    _buildFieldLabel(context, 'Assign To'),
                    SizedBox(height: 8.h),
                    DropdownButtonFormField<String>(
                      value: selectedUserId,
                      icon: const Icon(Icons.keyboard_arrow_down, color: Color(0xFF64748B)),
                      decoration: _buildInputDecoration(context, ''),
                      items: teamMembers.map((u) => DropdownMenuItem(value: u.id, child: Text(u.fullName))).toList(),
      ...
      builder: (ctx) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Review Task'.tr(context), style: TextStyle(fontSize: 18.sp, fontWeight: FontWeight.bold)),
              SizedBox(height: 4.h),
              Text(task.title, style: TextStyle(fontSize: 14.sp, color: AppColors.textSecondary)),
              SizedBox(height: 16.h),
              TextField(
                controller: feedbackController,
                maxLines: 3,
                decoration: InputDecoration(
                  hintText: 'Feedback / comments'.tr(context),
