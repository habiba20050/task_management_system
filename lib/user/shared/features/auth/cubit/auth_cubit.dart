  ...
  Future<void> checkAuthStatus() async {
    ...
    try {
      ...
      if (token != null && email != null) {
        final matches = MockDatabase.instance.users.where(
          (u) => u.email.toLowerCase() == email.trim().toLowerCase(),
        );
        if (matches.isNotEmpty) {
          final mockUser = matches.first;
    ...
    // Auto-login as Team Leader for development
    final defaultMock = MockDatabase.instance.users.firstWhere(
      (u) => u.role == 'Team Leader',
      orElse: () => MockDatabase.instance.users.first,
    );
    final defaultUser = UserModel(
      id: defaultMock.id,
      email: defaultMock.email,
      username: defaultMock.email.split('@')[0],
      fullName: defaultMock.fullName,
      role: defaultMock.role,
    );
    emit(AuthSuccess(defaultUser, 'mock_token_leader'));
  }
