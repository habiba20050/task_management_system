...
class _Filters {
  ...
  String priority = 'All';
  String status = 'All';
  String department = 'All'; // department name
  String userType = 'All'; // 'All' | 'Manager' | 'Team' | 'Group'
  String memberScope = 'Whole Team'; // 'Team Leader' | 'Manager' | 'Team Member' | 'Whole Team'
  String member = 'All'; // user id | team name
...
class _TasksPageState extends State<TasksPage> {
  ...
  // --- Fetch all tasks and apply the current filters + sorting ---
  List<UnifiedItem> _getTasks() {
    ...
    final filtered = items.where((item) {
      ...
      final matchesSearch = item.title.toLowerCase().contains(f.search.toLowerCase()) ||
          item.description.toLowerCase().contains(f.search.toLowerCase());

      final matchesPriority = f.priority == 'All' || item.priority.toUpperCase() == f.priority.toUpperCase();

      final matchesStatus = f.status == 'All' || item.status.toLowerCase() == f.status.toLowerCase();

      final matchesDepartment = f.department == 'All' ||
          task.taskDepartment == f.department ||
          task.assignedDepartment == f.department;

      // User Type determines the assignee type (reuses the previous tab categorization)
      bool matchesUserType = true;
      switch (f.userType) {
        case 'Manager':
          matchesUserType = task.assignedRole == 'Manager' || owner.role == 'Manager';
          break;
        case 'Team':
          matchesUserType = task.assignmentMode == 'Team' ||
              task.assignedTeamId != null ||
              task.taskType == 'Team Task';
          break;
        case 'Group':
          matchesUserType = task.taskType == 'Custom / Multiple' || task.customAssigneeIds.isNotEmpty;
          break;
      }

      // Member narrows down the assignee based on the selected User Type + For Whom scope
      bool matchesMember = f.member == 'All';
      if (!matchesMember) {
        if (f.userType == 'Group') {
          // Group → a person among the multiple assignees
          matchesMember = task.customAssigneeIds.contains(f.member);
        } else if (f.memberScope == 'Whole Team') {
          // 'All' | 'Team' + Whole Team → a specific team name
          matchesMember = item.teamName == f.member;
        } else {
          // Team Leader | Manager | Team Member → a specific user id
          matchesMember = task.currentOwnerId == f.member;
        }
      }
      ...
      if (f.dateRange != null) {
        ...
        if (taskDate != null) {
          matchesDateRange = taskDate.isAfter(f.dateRange!.start.subtract(const Duration(days: 1))) &&
              taskDate.isBefore(f.dateRange!.end.add(const Duration(days: 1)));
        }
      }

      return matchesSearch && matchesPriority && matchesStatus && matchesDepartment && matchesUserType && matchesMember && matchesDateRange;
    }).toList();
  ...
  Widget build(BuildContext context) {
    ...
    final tableItemsCount = tasks.length;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(
                  height: viewHeight,
                  child: _buildTasksView(tasks, role, userId),
                ),
              ],
            ),
          ),
        ...
        children: [
          ...
          Container(
            ...
            child: TextField(
              ...
              onChanged: (val) => setState(() => f.search = val),
            ),
          ),
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              const gap = 12.0;
              final showForWhom = f.userType == 'All' || f.userType == 'Team';
              ...
              // Member options depend on the selected User Type + For Whom scope
              final List<String> memberOptions;
              final Map<String, String>? memberLabelMap;
              final String memberDisplay;
              if (f.userType == 'Group') {
                memberOptions = ['All', ...db.users.map((u) => u.id)];
                memberLabelMap = {
                  'All': 'All'.tr(context),
                  ...{for (var u in db.users) u.id: u.fullName},
                };
                memberDisplay = f.member == 'All'
                    ? 'All'.tr(context)
                    : db.users.firstWhere((u) => u.id == f.member, orElse: () => db.users.first).fullName;
              } else if (f.memberScope == 'Whole Team') {
                memberOptions = ['All', ...db.teams.map((t) => t.name)];
                memberLabelMap = {'All': 'All'.tr(context)};
                memberDisplay = f.member == 'All' ? 'All'.tr(context) : f.member;
              } else {
                final roleUsers = db.users.where((u) => u.role == f.memberScope).toList();
                memberOptions = ['All', ...roleUsers.map((u) => u.id)];
                memberLabelMap = {
                  'All': 'All'.tr(context),
                  ...{for (var u in roleUsers) u.id: u.fullName},
                };
                memberDisplay = f.member == 'All'
                    ? 'All'.tr(context)
                    : db.users.firstWhere((u) => u.id == f.member, orElse: () => db.users.first).fullName;
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: _buildFilterField(
                      ...
                      value: f.status.tr(context),
                      options: ['All', 'Pending', 'Assigned', 'In Progress', 'Submitted', 'Under Review', 'Approved', 'Completed', 'Needs Changes', 'Reopened', 'Rejected', 'Overdue'],
                      onSelected: (val) => setState(() => f.status = val),
                    ),
                  ),
                  SizedBox(
                    width: fieldWidth,
                    child: _buildFilterField(
                      ...
                      value: f.department == 'All' ? 'All'.tr(context) : f.department,
                      options: ['All', ...db.departments.map((d) => d.name)],
                      onSelected: (val) => setState(() => f.department = val),
                    ),
                  ),
                  SizedBox(
                    width: fieldWidth,
                    child: _buildFilterField(
                      ...
                      value: f.userType == 'All' ? 'All'.tr(context) : f.userType.tr(context),
                      options: ['All', 'Manager', 'Team', 'Group'],
                      onSelected: (val) => setState(() {
                        if (f.userType != val) {
                          f.userType = val;
                          f.memberScope = val == 'Manager'
                              ? 'Manager'
                              : val == 'Group'
                                  ? 'Team Member'
                                  : 'Whole Team';
                          f.member = 'All'; // member scope changed → reset selection
                        }
                      }),
                    ),
                  ),
                  if (showForWhom)
                    SizedBox(
                      ...
                      child: _buildFilterField(
                        ...
                        value: f.memberScope.tr(context),
                        options: ['Team Leader', 'Manager', 'Team Member', 'Whole Team'],
                        onSelected: (val) => setState(() {
                          if (f.memberScope != val) {
                            f.memberScope = val;
                            f.member = 'All'; // scope changed → reset selection
                          }
                        }),
                      ),
                    ),
                  SizedBox(
                    ...
                    child: _buildFilterField(
                      ...
                      onSelected: (val) => setState(() => f.member = val),
                    ),
                  ),
                  SizedBox(
                    width: fieldWidth,
      ...
      children: [
        ...
        PopupMenuButton<String>(
          ...
          onSelected: (val) async {
            if (val == 'pick') {
              final range = await showDialog<DateTimeRange>(
                context: context,
                builder: (context) {
                  DateTime? start = f.dateRange?.start;
                  DateTime? end = f.dateRange?.end;
              ...
              if (range != null) setState(() => f.dateRange = range);
            } else if (val == 'clear') {
              setState(() => f.dateRange = null);
            }
          },
          itemBuilder: (context) => [
            PopupMenuItem(
          ...
          child: Container(
            ...
            padding: EdgeInsets.symmetric(horizontal: 12.w),
            decoration: BoxDecoration(
              color: f.dateRange != null
                  ? AppColors.primary.withValues(alpha: 0.08)
                  : const Color(0xFFF1F5F9),
              borderRadius: BorderRadius.circular(14.r),
              border: Border.all(
                color: f.dateRange != null ? AppColors.primary.withValues(alpha: 0.5) : Colors.transparent,
              ),
            ),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Expanded(
                  child: Row(
                    children: [
                      Icon(Icons.date_range, size: 16.sp, color: f.dateRange != null ? AppColors.primary : AppColors.textSecondary),
                      SizedBox(width: 6.w),
                      Expanded(
                        child: Text(
                          f.dateRange == null
                              ? 'Select'.tr(context)
                              : '${DateFormat('MM/dd').format(f.dateRange!.start)} - ${DateFormat('MM/dd').format(f.dateRange!.end)}',
                          style: TextStyle(fontSize: 12.5.sp, fontWeight: FontWeight.w600, color: f.dateRange != null ? AppColors.primary : AppColors.textPrimary),
                          overflow: TextOverflow.ellipsis,
                        ),
                      ),
                    ],
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildFieldLabel(context, 'Department', icon: Icons.business_outlined),
                      SizedBox(height: 10.h),
                      DropdownButtonFormField<String>(
                        initialValue: taskDept,
                        isExpanded: true,
                        icon: const Icon(Icons.keyboard_arrow_down_rounded, color: AppColors.textSecondary),
                        decoration: _buildInputDecoration(context, '', icon: Icons.business_outlined),
