...
class _EvaluationsPageState extends State<EvaluationsPage>
  ...
  Widget build(BuildContext context) {
    ...
    final allRoles = ['All', 'Manager', 'Team Leader', 'Team Member'];
    final allDepts = ['All', ...db.users.map((u) => u.department).toSet().toList()..sort()];
    final allUsers = db.users.where((u) => u.role != 'Admin').toList();

    final filteredUsers = allUsers.where((u) {
      if (_selectedRole != 'All' && u.role != _selectedRole) return false;
      if (_selectedDept != 'All' && u.department != _selectedDept) return false;
    ...
    final allLeaders = allUsers.where((u) => u.role == 'Team Leader').toList();
    final allMembers = allUsers.where((u) => u.role == 'Team Member').toList();

    final avgAll = allUsers.isEmpty
        ? 0.0
        : allUsers.map((u) => u.finalScore).reduce((a, b) => a + b) / allUsers.length;
    final bestUser = allUsers.isEmpty
        ? null
        : (List<MockUser>.from(allUsers)..sort((a, b) => b.finalScore.compareTo(a.finalScore))).first;
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 3.h),
                        Text(
                          'All Departments - Organization-wide Rankings and Scores'.tr(context),
                          style: TextStyle(
                            fontSize: 13.sp,
                        ...
                        child: ResponsiveLayout.isMobile(context)
                            ? Row(
                                ...
                                children: [
                                  ...
                                  SizedBox(width: 8),
                                  Text('Organization Overview'.tr(context)),
                                ],
                              )
                            : Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                                ...
                                children: [
                                  ...
                                  SizedBox(width: 8),
                                  Flexible(
                                    child: Text('Organization Overview'.tr(context),
                      ...
                      child: Padding(
                        ...
                        child: ResponsiveLayout.isMobile(context)
                            ? Row(
                                ...
                                crossAxisAlignment: CrossAxisAlignment.center,
                                children: [
                                  const Icon(Icons.filter_list_outlined, size: 16),
                                  SizedBox(width: 8),
                                  Text('Filter & Browse'.tr(context)),
                                ],
                              )
                            : Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                                ...
                                children: [
                                  ...
                                  SizedBox(width: 8),
                                  Flexible(
                                    child: Text('Filter & Browse'.tr(context),
                                        maxLines: 1, overflow: TextOverflow.ellipsis),
                                  ),
                                ],
                              ),
  ...
  Widget _buildOverviewTab(
    ...
    required List<MockUser> allManagers,
    required List<MockUser> allLeaders,
    required List<MockUser> allMembers,
    required double avgAll,
    required MockUser? bestUser,
  }) {
    final sortedAll = [...allManagers, ...allLeaders, ...allMembers]
      ..sort((a, b) => b.finalScore.compareTo(a.finalScore));
    final worstUser = sortedAll.length > 1 ? sortedAll.last : null;

    final sortedManagers = List<MockUser>.from(allManagers)..sort((a, b) => b.finalScore.compareTo(a.finalScore));
    final sortedLeaders = List<MockUser>.from(allLeaders)..sort((a, b) => b.finalScore.compareTo(a.finalScore));
    final sortedMembers = List<MockUser>.from(allMembers)..sort((a, b) => b.finalScore.compareTo(a.finalScore));
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
            ...
            return GridView(
              ...
              children: [
                StatCard(
                  ...
                  value: sortedAll.length.toString(),
                  icon: Icons.group_work_outlined,
                  accentColor: AppColors.primary,
                ),
                StatCard(
                  title: 'Managers'.tr(context),
                  value: allManagers.length.toString(),
                  icon: Icons.supervisor_account_outlined,
                  accentColor: Colors.purple,
                ),
                StatCard(
                  title: 'Avg Company Score'.tr(context),
                  value: '${avgAll.toStringAsFixed(1)}%',
                  icon: Icons.analytics_outlined,
                  accentColor: _getScoreColor(avgAll),
                ),
                StatCard(
                  title: 'Total Depts'.tr(context),
                  value: db.teams.map((t) => t.department).toSet().length.toString(),
          ...
          // Rankings
          _sectionTitle('Organization Rankings'.tr(context)),
          ...
          LayoutBuilder(builder: (context, constraints) {
            ...
            return GridView.builder(
              ...
              itemBuilder: (context, i) {
                switch (i) {
                  case 0:
                    return _rankingCard(context, 'Best Overall'.tr(context),
                        bestUser, Icons.emoji_events_outlined, Colors.amber);
                  default:
                    return _rankingCard(context, 'Needs Improvement'.tr(context),
                        worstUser, Icons.trending_down, AppColors.danger);
                }
              },
            );
          }),
          ...
          _sectionTitle('Managers'.tr(context)),
          SizedBox(height: 12.h),
          sortedManagers.isEmpty
              ? _emptyState(context, 'No managers found'.tr(context))
              : _buildMemberGrid(context, db, sortedManagers, currentUserId, isDesktop),
          SizedBox(height: 24.h),

          _sectionTitle('Team Leaders'.tr(context)),
          SizedBox(height: 12.h),
          sortedLeaders.isEmpty
              ? _emptyState(context, 'No team leaders found'.tr(context))
  ...
  Widget _buildFilterTab(
    ...
    bool isDesktop,
    String currentUserId, {
    required List<MockUser> filteredUsers,
    required List<String> allRoles,
    required List<String> allDepts,
  }) {
    return Column(
      children: [
        // Filter bar
          ...
          child: Column(
            children: [
                    ...
                    child: Container(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          ...
                          items: allRoles.map((r) => DropdownMenuItem(value: r, child: Text(r.tr(context)))).toList(),
                        ),
                      ),
                    ),
                  ),
                    ...
                    child: Container(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          ...
                          items: allDepts.map((d) => DropdownMenuItem(value: d, child: Text(d.tr(context)))).toList(),
                        ),
                      ),
                    ),
                  ),
              ...
              TextField(
                ...
                decoration: _fieldDecoration('Search by name or department...'.tr(context)).copyWith(
        ...
        Expanded(
          child: filteredUsers.isEmpty
              ? _emptyState(context, 'No users match your criteria.'.tr(context))
              : SingleChildScrollView(
                  child: _buildMemberGrid(context, db, filteredUsers, currentUserId, isDesktop),
                ),
        ),
      ],
    );
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 2.h),
                      Text(
                        '${u.role.tr(context)} · ${u.department.tr(context)}',
